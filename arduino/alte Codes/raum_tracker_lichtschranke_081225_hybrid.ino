...
// ===== RESET STATE MIT DEBOUNCE (WICHTIG FÜR WIFI-STACK!) =====
void resetState() {
  state = IDLE;
  delay(debounceTime);  // 800ms Pause - kritisch für WiFi-Hintergrundaufgaben!
}
...
void loop() {
  ...
  if (WiFi.status() != WL_CONNECTED) {
    ...
    connectWiFi();
    delay(5000);
    resetState();  // State zurücksetzen nach Reconnect
    return;
  }
  ...
  delay(50);  // Erhöht von 10ms auf 50ms - wichtig für WiFi-Stack!
}
