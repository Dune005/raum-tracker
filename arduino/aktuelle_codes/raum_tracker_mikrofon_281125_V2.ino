...
const float MIC_FULL_SCALE = 8388608.0;   // 2^23 für 24-bit I2S Daten
const float MIC_OFFSET_DB = 120.0;        // 0 dBFS ≈ 120 dB SPL (INMP441: -26 dBFS @ 94 dB)
const float MIN_NORMALIZED_LEVEL = 1e-8;  // Schutz gegen log10(0)
bool isCalibrated = false;
float baselineNoise = 0.0;
float quietNoiseDb = QUIET_ROOM_DB;
...
void setup() {
  ...
  Serial.printf("Bereich: %.0f-%.0f dB SPL\n\n", QUIET_ROOM_DB, LOUD_ROOM_DB);
}
...
void calibrateMicrophone() {
  ...
  if (validSamples > 10) {
    ...
    isCalibrated = true;
    Serial.println("\n✅ Kalibrierung OK");
    float baselineDb = MIC_OFFSET_DB + (20.0 * log10(max(baselineNoise / MIC_FULL_SCALE, MIN_NORMALIZED_LEVEL)));
    quietNoiseDb = max(QUIET_ROOM_DB, baselineDb);
    Serial.printf("   Baseline: %.0f | Min: %.0f | Max: %.0f | Ruhepegel: %.1f dB SPL\n", 
                  baselineNoise, minVal, maxVal, quietNoiseDb);
  } else {
    baselineNoise = 30000;
...
float getRawAudioLevel() {
  ...
  for (int i = 0; i < sample_count; i++) {
    // ← INMP441 gibt 24-bit Samples, die in 32-bit Frames linksbündig liegen
    int32_t sample32 = samples[i] >> 8;   // Hole die 24-bit Nutzdaten
...
float rmsToDbSpl(float rmsValue) {
  float normalized = rmsValue / MIC_FULL_SCALE;
  normalized = max(normalized, MIN_NORMALIZED_LEVEL);
  float dBFS = 20.0 * log10(normalized);
  return MIC_OFFSET_DB + dBFS;
}
...
float readSoundLevel() {
  ...
  float rawLevel = getRawAudioLevel();
  float db = rmsToDbSpl(rawLevel);
  
  if (db < quietNoiseDb) db = quietNoiseDb;
  if (db > LOUD_ROOM_DB) db = LOUD_ROOM_DB;
  
  float smoothed = (currentSoundLevel * 0.3) + (db * 0.7); // Schnellere Reaktion
  return smoothed;
}
